  ...
  return (
    <View style={tw.style(`rounded-[30px] relative overflow-hidden bg-opacity-65 ${bgColor}`)}>
      <View style={tw`px-6 py-6`}>
        ...
        <BadgeButton size="sm" style={tw`px-10 py-3 mt-8`} onPress={onPress}>
          {cta}
        </BadgeButton>
      </View>
      <View
        style={tw.style(`absolute w-16 h-44 -top-16 right-8 bg-purple-200 ${bgColor}`, {
          transform: [{ rotate: "30deg" }],
        })}
      />
      <View
        style={tw.style(`absolute w-16 h-44 -bottom-10 right-6 ${bgColor}`, {
          transform: [{ rotate: "30deg" }],
        })}
      />
    </View>
